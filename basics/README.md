...
* The ```#!/bin/bash``` header, also known as "shabang" (= sharp (#) + bang (!)), is used to define the file as a bash script, and not a binary (bin) file. Scripts are executable by using the ```./filename``` syntax ;
...
* Due to owner of the file not necessarily corresponding to the user of the testing machine, there is a need to give an execution permission to users in the scripts' files. This is all done with the following command (there is no need to understand that command for now) :

   ```chmod u+x X-file_name```
